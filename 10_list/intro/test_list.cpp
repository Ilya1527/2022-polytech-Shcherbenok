...
TEST_CASE("Elements can be removed") 
{
	...
	// удаляем число с места с индексом 3
	ll.remove(3);
	CHECK(ll == LinkedList<int>{ 8, 10, 20, 30, 40, 50, 60 });
	// удаляем число с места с индексом 0
	ll.remove(0);
	CHECK(ll == LinkedList<int>{ 10, 20, 30, 40, 50, 60 });
	// удаляем число с места с индексом 5
	ll.remove(5);
...
TEST_CASE("Deep copying") 
{
	...
	// клонируем список ll в список ll_cloned
	ll.clone(&ll_cloned);
